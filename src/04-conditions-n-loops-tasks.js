...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startSymbol}${lower}, ${upper}${endSymbol}`;
}
...
/* function reverseInteger(num) {
  const isNegative = num < 0; // Проверяем, отрицательное ли число
  const reversedNum = +num.toString().split('').reverse().join(''); // Переворачиваем число
  return isNegative ? -reversedNum : reversedNum; // Возвращаем результат с учётом знака
} */
...
function evaluateTicTacToePosition(position) {
  ...
  const winner = winningCombinations.find((combination) => (
    combination[0] === combination[1] && combination[1] === combination[2] && combination[0]
  ));
